...
import com.mormon.net.entities.Conversation;
import com.mormon.net.entities.Message;
import com.mormon.net.entities.MucOptions;
import com.mormon.net.http.HttpConnectionManager;
import com.mormon.net.services.MessageArchiveService;
...
import com.mormon.net.xmpp.OnMessagePacketReceived;
import com.mormon.net.xmpp.chatstate.ChatState;
import com.mormon.net.xmpp.jid.Jid;
import com.mormon.net.xmpp.pep.Avatar;
...
public class MessageParser extends AbstractParser implements OnMessagePacketReceived {
	...
	private Message parseChat(MessagePacket packet, Account account) {
		...
		if (pgpBody != null) {
			...
			finishedMessage = new Message(conversation,pgpBody, Message.ENCRYPTION_PGP, Message.STATUS_RECEIVED);
			
		} else {
			
			finishedMessage = new Message(conversation,packet.getBody(), Message.ENCRYPTION_NONE,Message.STATUS_RECEIVED);
		...
		if (conversation.getMode() == Conversation.MODE_MULTI && !jid.isBareJid()) {
			...
			finishedMessage.setTrueCounterpart(conversation.getMucOptions().getTrueCounterpart(jid.getResourcepart()));
	...
	private Message parseGroupchat(MessagePacket packet, Account account) {
		...
		if (mXmppConnectionService.find(account.pendingConferenceLeaves,account, from.toBareJid()) != null) {
			
			return null;
			
		}
		
		Conversation conversation = mXmppConnectionService.findOrCreateConversation(account, from.toBareJid(), true);
		...
		if (from.getResourcepart().equals(conversation.getMucOptions().getActualNick())) {
			...
			if (mXmppConnectionService.markMessage(conversation,packet.getId(), Message.STATUS_SEND_RECEIVED)) {
		...
		if (pgpBody == null) {
			...
			finishedMessage = new Message(conversation,packet.getBody(), Message.ENCRYPTION_NONE, status);
			
		} else {
			
			finishedMessage = new Message(conversation, pgpBody,Message.ENCRYPTION_PGP, status);
		...
		if (status == Message.STATUS_RECEIVED) {
			...
			finishedMessage.setTrueCounterpart(conversation.getMucOptions().getTrueCounterpart(from.getResourcepart()));
	...
	private Message parseCarbonMessage(final MessagePacket packet, final Account account) {
		...
		if (packet.hasChild("received", "urn:xmpp:carbons:2")) {
			...
			forwarded = packet.findChild("received", "urn:xmpp:carbons:2").findChild("forwarded", "urn:xmpp:forward:0");
			status = Message.STATUS_RECEIVED;
			
		} else if (packet.hasChild("sent", "urn:xmpp:carbons:2")) {
			
			forwarded = packet.findChild("sent", "urn:xmpp:carbons:2").findChild("forwarded", "urn:xmpp:forward:0");
			status = Message.STATUS_SEND;
		...
		if (!message.hasChild("body")) {
			...
			} else if (status == Message.STATUS_SEND && message.hasChild("displayed", "urn:xmpp:chat-markers:0")) {
				
				final Jid to = message.getAttributeAsJid("to");
				
				if (to != null) {
					
					final Conversation conversation = mXmppConnectionService.find(mXmppConnectionService.getConversations(), account,to.toBareJid());
		...
		if (message.hasChild("x","http://jabber.org/protocol/muc#user") && "chat".equals(message.getAttribute("type"))) {
			
			return null;
		}
		
		Conversation conversation = mXmppConnectionService.findOrCreateConversation(account, fullJid.toBareJid(), false);
		...
		if (pgpBody != null) {
			...
			finishedMessage = new Message(conversation, body,Message.ENCRYPTION_NONE, status);
		...
		if (conversation.getMode() == Conversation.MODE_MULTI && !fullJid.isBareJid()) {
			
			finishedMessage.setType(Message.TYPE_PRIVATE);
			finishedMessage.setTrueCounterpart(conversation.getMucOptions().getTrueCounterpart(fullJid.getResourcepart()));
	...
	private void parseNonMessage(Element packet, Account account) {
		...
		} else if (from != null && packet.hasChild("received", "urn:xmpp:chat-markers:0")) {
			
			String id = packet.findChild("received", "urn:xmpp:chat-markers:0").getAttribute("id");
			updateLastseen(packet, account, false);
			mXmppConnectionService.markMessage(account, from.toBareJid(),id, Message.STATUS_SEND_RECEIVED);
			
		} else if (from != null && packet.hasChild("received", "urn:xmpp:receipts")) {
			
			String id = packet.findChild("received", "urn:xmpp:receipts").getAttribute("id");
			updateLastseen(packet, account, false);
			mXmppConnectionService.markMessage(account, from.toBareJid(),id, Message.STATUS_SEND_RECEIVED);
	...
	public void onMessagePacketReceived(Account account, MessagePacket packet) {
		...
		if ((packet.getType() == MessagePacket.TYPE_CHAT || packet.getType() == MessagePacket.TYPE_NORMAL)) {
			...
			} else if (packet.hasChild("received", "urn:xmpp:carbons:2") || (packet.hasChild("sent", "urn:xmpp:carbons:2"))) {
